...
class Staff(Base, TimestampMixin):
    ...
    id: Mapped[str] = mapped_column(
        UUID, primary_key=True, index=True, default=lambda: str(uuid4())
    )
    ...
    received_parcels = relationship(
        "Parcel", back_populates="received_by", foreign_keys="Parcel.received_by_id"
    )
    assignments_made = relationship(
        "Assignment", back_populates="assigned_by_staff", foreign_keys="Assignment.assigned_by_staff_id"
    )
    refunds_approved = relationship(
        "Refund", back_populates="approved_by_staff", foreign_keys="Refund.approved_by_staff_id"
    )
...
class Parcel(Base, TimestampMixin):
    ...
    received_by_id: Mapped[str] = mapped_column(
        UUID, ForeignKey("staff.id", ondelete="RESTRICT"), nullable=False
    )
    ...
    current_status: Mapped[ParcelStatus] = mapped_column(
        SAEnum(ParcelStatus), default=ParcelStatus.RECEIVED, nullable=False
    )
    delivery_outcome: Mapped[DeliveryOutcome] = mapped_column(
        SAEnum(DeliveryOutcome), default=DeliveryOutcome.PENDING, nullable=False
    )
...
class ParcelPhoto(Base, TimestampMixin):
    ...
    parcel_id: Mapped[str] = mapped_column(
        UUID, ForeignKey("parcels.id", ondelete="CASCADE"), index=True
    )
...
class Assignment(Base, TimestampMixin):
    ...
    rider_id: Mapped[str] = mapped_column(
        UUID, ForeignKey("riders.id", ondelete="RESTRICT"), index=True
    )
    assigned_by_staff_id: Mapped[str] = mapped_column(
        UUID, ForeignKey("staff.id", ondelete="RESTRICT"), index=True
    )
...
class DeliveryAttempt(Base, TimestampMixin):
    ...
    rider_id: Mapped[Optional[str]] = mapped_column(
        UUID, ForeignKey("riders.id", ondelete="SET NULL"), index=True, nullable=True
    )
    status: Mapped[DeliveryAttemptStatus] = mapped_column(
        SAEnum(DeliveryAttemptStatus), nullable=False
    )
...
class Receipt(Base, TimestampMixin):
    ...
    parcel_id: Mapped[str] = mapped_column(
        UUID, ForeignKey("parcels.id", ondelete="CASCADE"), unique=True, index=True
    )
...
class Dispute(Base, TimestampMixin):
    ...
    status: Mapped[DisputeStatus] = mapped_column(
        SAEnum(DisputeStatus), default=DisputeStatus.OPEN, nullable=False
    )
...
class Refund(Base, TimestampMixin):
    ...
    dispute_id: Mapped[str] = mapped_column(
        UUID, ForeignKey("disputes.id", ondelete="CASCADE"), unique=True, index=True
    )
    amount: Mapped[float] = mapped_column(Float, nullable=False)
    currency: Mapped[str] = mapped_column(String(8), nullable=False, default="USD")
    approved_by_staff_id: Mapped[str] = mapped_column(
        UUID, ForeignKey("staff.id", ondelete="RESTRICT"), nullable=False
    )
